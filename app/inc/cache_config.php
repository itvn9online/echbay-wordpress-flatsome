...
if ( mtv_id > 0 || !file_exists( $__eb_txt_only_conf ) ) {
    ...
    */
    ...
    // daidq 2021-07-24 -> tạm thời tắt suppercache
    if ( 1 == 2 && mtv_id > 0 && $__cf_row[ 'cf_enable_ebsuppercache' ] == 1 ) {
        $content_of_wp_index = trim( file_get_contents( ABSPATH . 'index.php', 1 ) );

        // nếu chưa có file cache thì thêm vào thôi
        if ( strstr( $content_of_wp_index, '/ebcache.php' ) == false ) {
            //echo __DIR__ . '<br>' . "\n";

            // tách theo dấu xuống dòng \n
            $content_of_wp_index = explode( "\n", $content_of_wp_index );
            if ( trim( $content_of_wp_index[ 0 ] ) == '<?php' ) {
                $content_of_wp_index[ 0 ] .= ' ' . "\n" . 'include __DIR__ . \'/' . str_replace( ABSPATH, '', EB_THEME_PLUGIN_INDEX ) . 'ebcache.php\';';
                ...
                //
                if ( trim( $content_of_wp_index[ 0 ] ) == '<?php' ) {
                    $content_of_wp_index[ 0 ] .= ' ' . 'include __DIR__ . \'/' . str_replace( ABSPATH, '', EB_THEME_PLUGIN_INDEX ) . 'ebcache.php\';';
