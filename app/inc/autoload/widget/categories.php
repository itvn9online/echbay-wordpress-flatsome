...
class ___echbay_widget_list_current_category extends WP_Widget {
    ...
    function form( $instance ) {
        ...
        */
        ...
        //
        if ( !isset( $instance[ 'cat_ids' ] ) || $instance[ 'cat_ids' ] == 0 ) {
            $input_name = $this->get_field_name( 'get_current_cid' );
            //echo $instance[ 'get_current_cid' ];

            _eb_widget_echo_widget_input_checkbox( $input_name, $instance[ 'get_current_cid' ], 'Lấy các nhóm con của chính danh mục đang được xem &rarr; <strong>$cid</strong> sẽ được chỉ định để thay thế cho <strong>$cat_ids</strong> (dùng khi truy cập danh mục cụ thể)' );


            // chức năng get_parent chỉ hoạt động khi get_current_cid = OFF
            if ( !isset( $instance[ 'get_current_cid' ] ) || $instance[ 'get_current_cid' ] != 'on' ) {
                $input_name = $this->get_field_name( 'get_parent' );
                //echo $instance[ 'get_child' ];

                _eb_widget_echo_widget_input_checkbox( $input_name, $instance[ 'get_parent' ], 'Tự tìm các nhóm cùng cha &rarr; <strong>$parent_cid</strong> sẽ được chỉ định để thay thế cho <strong>$cat_ids</strong> (thường dùng cho phần chi tiết sản phẩm, chi tiết bài viết)' );
            }
        }
    ...
    function widget( $args, $instance ) {
        ...
        $get_current_cid = isset( $instance[ 'get_current_cid' ] ) ? $instance[ 'get_current_cid' ] : 'off';
        //$get_current_cid = ( $get_current_cid == 'on' ) ? true : false;
        ...
        if ( $cat_ids == 0 ) {
            if ( $get_current_cid != 'off' ) {
                $cat_ids = $cid;
            } else if ( $get_parent != 'off' ) {
                //global $cid;
                global $parent_cid;
                //global $pid;
